...
#'
#' @param x Table of mass, normalised density, and density
...
#' @export
plot_CSS <- function(.df, normalised = T, logbase = 2){
  .df %>%
    dplyr::mutate(y = norm_density*normalised + density*abs(normalised-1)) %>%
    ggplot2::ggplot() +
